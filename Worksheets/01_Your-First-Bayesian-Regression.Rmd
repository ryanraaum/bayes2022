...
That looks fine for linear regression modeling and there is an apparent difference between the sexes and time periods.
...
```{r center-logheight}
tusksize <- tusksize %>% 
  mutate(logheight = scale(logheight, center = TRUE, scale = FALSE))
```
...
Now we can fit a Bayesian regression model. We'll use the `stan_lm` function from `rstanarm` with a `R2` prior. In the function call below, replace `FIX` with a good starting value for the likely R-squared of the model. 

```{r rstanarm-regression}
fit2 <- stan_lm(tuskformula1,
                data = tusksize,
                #prior = R2(FIX, what="mean"),
                prior = R2(0.87, what="mean"),
...
First, we can get an overview of some of the sampler diagnostics using the `check_hmc_diagnostics` function from `rstan`. 
