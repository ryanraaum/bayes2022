...
There might be a divergent transition when you first run the code above. If so, you should fix that by changing the `adapt_delta` value. The default value for `adapt_delta` for `brms` is 0.8, which is quite low compared to the `rstanarm` default value of 0.95.
